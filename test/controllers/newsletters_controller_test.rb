...
class NewslettersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create newsletter" do
    assert_difference('Newsletter.count') do
      post newsletters_url, params: { newsletter: { email: @newsletter.email, name: @newsletter.name } }
    end
  ...
  test "should update newsletter" do
    patch newsletter_url(@newsletter), params: { newsletter: { email: @newsletter.email, name: @newsletter.name } }
